...
const randomCount = 0;
const inviteCodes = [
  `f0RpMb0@ZE9wNI3jJrZUgBWHjyw@IBIzaO2zYv0792zcy3oV0qA`,
  `f0RpMb0@ZE9wNI3jJrZUgBWHjyw@IBIzaO2zYv0792zcy3oV0qA`,
]
if ($.isNode()) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/lonma/Jdscripts/master/cashbackup.json/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
